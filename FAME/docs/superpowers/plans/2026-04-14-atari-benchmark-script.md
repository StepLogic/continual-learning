# Atari Continual Learning Benchmark Script Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a single bash script that reproduces the FAME paper's (ICLR 2026) Atari continual learning experiments end-to-end: training all 6 algorithms on 2 games with 3 seeds, full cross-task evaluation, and results processing.

**Architecture:** A single `benchmark_experiments.sh` with 3 phases (train → evaluate → process). Training is sequential per game/algorithm/seed/mode. Evaluation tests every trained agent on all game modes. Results processing extracts TensorBoard data and computes continual RL metrics.

**Tech Stack:** Bash, Python (run_experiments.py, run_ppo_FAME.py, test_agent.py, process_results_pre.py, process_results.py), PyTorch, Gymnasium/ALE

---

### Task 1: Write the benchmark script — configuration and CLI parsing

**Files:**
- Create: `Atari/benchmark_experiments.sh`

- [ ] **Step 1: Write script header, CLI parsing, and configuration defaults**

Create `Atari/benchmark_experiments.sh` with the following content:
...
```bash
chmod +x Atari/benchmark_experiments.sh
```

- [ ] **Step 3: Commit**

```bash
git add Atari/benchmark_experiments.sh
git commit -m "feat: add benchmark script skeleton with CLI parsing and config"
```

---

### Task 2: Write Phase 1 — Training loop

**Files:**
- Modify: `Atari/benchmark_experiments.sh`

- [ ] **Step 1: Add the training phase to the benchmark script**

Append the training phase function after the helpers section:
...
run_training() {
    ...
    for env in "${ENV_LIST[@]}"; do
        ...
        local game_name=$(echo "$env" | sed 's/ALE\///;s/-v5//')  # e.g., Freeway, SpaceInvaders

        for algo in "${ALGO_LIST[@]}"; do
            # Map CLI algorithm name to model_type used by run_experiments.py
            local model_type="$algo"
            if [ "$algo" = "cnn-simple-ft" ]; then
                model_type="finetune"  # run_experiments.py uses "finetune" internally
            fi
            ...
            for seed in "${SEED_LIST[@]}"; do
                ...
                for mode in $(seq 0 $((n_modes - 1))); do
                    ...
                    local prev_args=$(prev_units_args "$env_prefix" "$algo" "$mode" "$seed" "$n_modes")
                    if [ -n "$prev_args" ]; then
                        cmd="$cmd $prev_args"
                    fi
...
```bash
git add Atari/benchmark_experiments.sh
git commit -m "feat: add Phase 1 training loop to benchmark script"
```

---

### Task 3: Write Phase 2 — Evaluation loop

**Files:**
- Modify: `Atari/benchmark_experiments.sh`

- [ ] **Step 1: Add the evaluation phase to the benchmark script**

Append the evaluation phase function after the training function:
...
```bash
git add Atari/benchmark_experiments.sh
git commit -m "feat: add Phase 2 evaluation loop to benchmark script"
```

---

### Task 4: Write Phase 3 — Results processing and main entry point

**Files:**
- Modify: `Atari/benchmark_experiments.sh`

- [ ] **Step 1: Add the results processing phase and main entry point**

Append after the evaluation function:
...
run_processing() {
    ...
    for env in "${ENV_LIST[@]}"; do
        ...
        for algo in "${ALGO_LIST[@]}"; do
            for seed in "${SEED_LIST[@]}"; do
                local exp_suffix="run_ppo"
                if [ "$algo" = "FAME" ]; then
                    exp_suffix="run_ppo_FAME"
                fi

                for mode in $(seq 0 $((n_modes - 1))); do
                    local event_dir="runs/ALE-${game_name}-v5_${mode}__${algo}__${exp_suffix}__${seed}"
                    if [ -d "$event_dir" ]; then
                        log "  Processing $event_dir"
                        python3 process_results_pre.py 2>&1 | tail -1 || \
                            log_error "  process_results_pre.py failed for $event_dir"
                    fi
                done
            done
        done
...
```bash
git add Atari/benchmark_experiments.sh
git commit -m "feat: add Phase 3 results processing and main entry point"
```

---

### Task 5: Fix `run_experiments.py` subprocess error handling

**Files:**
- Modify: `Atari/run_experiments.py:103`

The current `run_experiments.py` splits the command string on spaces (`cmd.split(" ")`), which breaks for `--prev-units agents/path1 agents/path2` since the space inside the prev-units paths would be incorrectly split. Must use `shell=True` or pass as a list.

- [ ] **Step 1: Fix the subprocess call to handle multi-word arguments correctly**

In `Atari/run_experiments.py`, change line 103:
...
This is necessary because `--prev-units agents/path1 agents/path2` contains spaces that should not be split. `shell=True` lets the shell handle the argument parsing.

- [ ] **Step 2: Commit**

```bash
git add Atari/run_experiments.py
git commit -m "fix: use shell=True in subprocess call to handle multi-word CLI args"
```

---

### Task 6: Fix `process_results.py` eval CSV column name match

**Files:**
- Modify: `Atari/process_results.py:393`

The benchmark script writes eval CSVs with column `train_mode` and `test_mode`, but `process_results.py` line 393 references `df["test mode"]` (with a space). Must ensure column names match.

- [ ] **Step 1: Update process_results.py to use underscore column names**

In `Atari/process_results.py`, change the `process_eval` function (line 393) from:
...
```python
sel = df[
    (df["test_mode"] == task_id)
    & (df["environment"] == env)
    & (df["algorithm"] == method)
]
```

Also check all other references to `"test mode"` or `"train mode"` in the file and update them to use underscores to match the benchmark script's eval CSV format.

- [ ] **Step 2: Commit**

```bash
git add Atari/process_results.py
git commit -m "fix: update column names in process_results.py to match benchmark eval CSV format"
```

---

### Task 7: Verify the script works in quick-test mode

**Files:**
- No file changes

- [ ] **Step 1: Run a quick smoke test to verify the script logic**

```bash
cd Atari
./benchmark_experiments.sh --quick 2>&1 | head -30
```

Expected: Script starts, prints configuration, and begins training cnn-simple on Freeway mode 0 with 10000 timesteps. May take a few minutes.

- [ ] **Step 2: Verify the --help flag works**

```bash
cd Atari
./benchmark_experiments.sh --help
```

Expected: Prints the usage documentation.

- [ ] **Step 3: Commit final state if any fixes were needed**

```bash
git add Atari/benchmark_experiments.sh
git commit -m "fix: address issues found in quick-test verification"
```

---

### Task 8: Update BENCHMARKING.md documentation

**Files:**
- Modify: `Atari/BENCHMARKING.md`

- [ ] **Step 1: Update BENCHMARKING.md to reflect the new script**

Replace the content of `Atari/BENCHMARKING.md` with updated documentation that covers the full reproduction script:
...
```bash
git add Atari/BENCHMARKING.md
git commit -m "docs: update BENCHMARKING.md for full reproduction script"
```