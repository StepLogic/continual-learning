...
def parse_args():
    ...
    parser.add_argument("--dino-size", type=str, choices=["s", "b", "l", "g"], default="s", help="DINO model size")
    parser.add_argument("--timesteps", type=int, default=100000, help="Total timesteps per task (default: 100k for limited resources, use 1e6 for full runs)")
...
if args.algorithm == "cnn-simple":
    ...
    model_type = "cnn-simple"
elif args.algorithm == "dino-simple":
    model_type = "dino-simple"
...
for i, task_id in enumerate(modes[first_idx:]):
    ...
    if args.algorithm == "dino-simple":
        params += f" --dino-size={args.dino_size}"
        params += " --num-envs=1"  # Limited GPU memory (6GB), use single env
    ...
    # FAME uses a different training script
    script = "run_ppo_FAME.py" if args.algorithm == "fame" else "run_ppo.py"
    ...
    if res.returncode != 0:
        print(f"*** Process returned code {res.returncode}. Continuing to next task.")
