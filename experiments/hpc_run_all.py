"""
HPC Continual RL Benchmark Experiment Runner.
...
Designed for HPC clusters with headless rendering (MUJOCO_GL=egl).

Usage:
    MUJOCO_GL=egl python experiments/hpc_run_all.py --method all --seeds 0 1 2
    MUJOCO_GL=egl python experiments/hpc_run_all.py --method base --env continual_world
"""
...
# Set headless rendering BEFORE importing mujoco/gymnasium
...
def get_agent_config(method_name: str) -> dict:
    ...
    HPC-optimized: 1M replay buffer, larger batches for 150GB GPU memory.
    """
    configs = {
        "base": {
            "hidden_dim": 512,
...
def run_evaluation_or_training(
    ...
    logger = get_logger()
    logger.info(
        f"Starting HPC evaluate_or_train: {method_name} on {env_name} (seed={seed})"
    )
    logger.info(f"MUJOCO_GL={os.environ.get('MUJOCO_GL', 'default')}")
...
def main():
    parser = argparse.ArgumentParser(
        description="HPC Continual RL Benchmark Runner with Checkpoint Resume"
    )
    parser.add_argument(
        "--method",
        type=str,
    ...
    parser.add_argument(
        "--render",
        action="store_true",
        help="Enable rendering (disable for HPC)",
    )
    parser.add_argument(
        "--evaluate-only",
        action="store_true",
    ...
    logger.info("=" * 60)
    logger.info("HPC Continual RL Benchmark")
    logger.info("=" * 60)
    logger.info(f"MUJOCO_GL={os.environ.get('MUJOCO_GL', 'default')}")
    logger.info(f"PYOPENGL_PLATFORM={os.environ.get('PYOPENGL_PLATFORM', 'not set')}")
    ...
    results_path = os.path.join(
        args.output_dir, f"hpc_results_{timestamp}.json"
    )
