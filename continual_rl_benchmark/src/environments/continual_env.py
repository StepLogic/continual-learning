...
class ContinualEnv:
    ...
    def _create_env(self, task_name: str) -> gym.Env:
        ...
        if self.env_name in ["metaworld", "continual_world"]:
            ...
            env = gym.make(f"Meta-World/MT1", env_name=task_name, render_mode=self.render_mode)
            return env
        elif self.env_name == "procgen":
            return gym.make(f"procgen:procgen-{task_name}-v0", render_mode=self.render_mode)
        else:
